...
class ShadowModels:
    ...
    def __init__(
        ...
        y: np.ndarray,
        n_models: int,
        input_data_dim_x: int,
        input_data_dim_y: int,
        target_classes: int,
        learner,
        **fit_kwargs,
    ) -> None:
        ...
        self.y = y
        self.input_data_dim_x = input_data_dim_x
        self.input_data_dim_y = input_data_dim_y
    ...
    def train_predict_shadows(self, **fit_kwargs):
        ...
        for model, data_subset in tqdm_notebook(zip(self.models, self._splits)):
            ...
            X_train = np.reshape(X_train, (-1,self.input_data_dim_x, self.input_data_dim_y))
            model.fit(X_train, y_train, **fit_kwargs)
            # data IN training set labelet 1
            y_train = y_train.reshape(-1, 1)
            predict_in = model.predict(X_train)
            res_in = np.hstack((predict_in, y_train, np.ones_like(y_train)))

            # data OUT of training set, labeled 0
            y_test = y_test.reshape(-1, 1)
            X_test = np.reshape(X_test, (-1,self.input_data_dim_x, self.input_data_dim_y))
